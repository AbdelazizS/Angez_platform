    ...
    protected function getRelevanceScoreSQL(string $searchQuery, string $language): string
    {
        if (empty($searchQuery)) {
            ...
            return "(
                ...
                (orders * 0.15) +
                (views * 0.01) +
                (SELECT COALESCE(AVG(rating), 0) * 20 FROM reviews 
                 WHERE reviews.order_id IN (SELECT id FROM orders WHERE orders.service_id = services.id)) +
                (SELECT COALESCE(freelancer_profiles.average_rating, 0) * 20 FROM freelancer_profiles 
                 WHERE freelancer_profiles.user_id = services.user_id)
            ) AS relevance_score";
        }
        ...
        $scoreParts[] = "(SELECT COALESCE(AVG(rating), 0) * 20 FROM reviews 
                         WHERE reviews.order_id IN (SELECT id FROM orders WHERE orders.service_id = services.id)";
        $scoreParts[] = "(SELECT COALESCE(freelancer_profiles.average_rating, 0) * 20 FROM freelancer_profiles 
                         WHERE freelancer_profiles.user_id = services.user_id)";
